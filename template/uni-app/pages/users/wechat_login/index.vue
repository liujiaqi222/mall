<template>
  <view :style="colorStyle" class="wrapper">
    ...
    <view class="wechat_login">
      <view class="btn-wrapper">
        ...
        <template v-if="configData.wechat_auth_switch">
          <button class="bg-theme btn1" v-if="bindPhone" open-type="getPhoneNumber" @getphonenumber="getphonenumber">{{
    $t(`授权登录`) }}</button>
          <button class="bg-theme btn1" v-else-if="!bindPhone" @click="getAuthLogin">
            {{ $t(`授权登录`) }}
          </button>
        </template>
        <button v-if="configData.phone_auth_switch" hover-class="none" @click="phoneLogin" class="btn2">{{ $t(`手机号登录`)
          }}</button>
        <!-- #endif -->
      </view>
    </view>
    ...
    <block v-if="isUp">
      <mobileLogin :isUp="isUp" :canClose="canClose" @close="maskClose" :authKey="authKey" @wechatPhone="wechatPhone">
      </mobileLogin>
    </block>
