  ...
  <view class="swiperBg" :style="{ marginTop: mt + 'rpx' }">
    ...
    <block v-if="isShow && imgUrls.length">
      <view class="swiper square" v-if="imgUrls.length">
        <swiper class="skeleton-rect" :style="'height:' + (imageH) + 'rpx;'" indicator-dots="true" :autoplay="true"
          :circular="circular" :interval="interval" :duration="duration" indicator-color="rgba(255,255,255,0.6)"
          ...
          <block v-for="(item, index) in imgUrls" :key="index">
            <swiper-item class="" :class="{ active: index == swiperCur }">
              <view @click="goDetail(item)" class='slide-navigator acea-row row-between-wrapper'>
                <image :src="item.img" class="slide-image" mode="widthFix" :style="'height:' + (imageH) + 'rpx;'">
                </image>
              </view>
            </swiper-item>
          </block>
