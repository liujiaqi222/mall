          ...
          <view class="bg-color">
            <view class='nav acea-row row-between-wrapper'>
              <view class='money'>{{ $t(`￥`) }}<text class='num'>{{ storeInfo.price || '' }}</text>
                <text
                  v-if="attribute.productAttr.length && (attribute.productAttr.length ? attribute.productAttr[0].attr_values.length : 0) > 1">{{
    $t(`起`) }}</text>
              ...
              <view class='acea-row row-middle'>
                <view class='timeItem' v-if="status == 1">
                  ...
                  <countDown :is-day="false" :tip-text="' '" :day-text="' '" :hour-text="' : '" :minute-text="' : '"
                    :second-text="' '" :datatime="datatime" style="margin-top: 4rpx;"></countDown>
                </view>
    ...
    attrValue
      ...
      <view class='footer acea-row row-between-wrapper' :class="{ 'eject': storeInfo.id }">
        ...
        <view class="bnt acea-row"
          ...
          <view class="buy bnts" :class="!storeInfo.product_is_show ? 'long-btn' : ''" @tap="goCat">{{ $t(`立即购买`) }}
          </view>
        </view>
        <view class="bnt acea-row"
          ...
          <view class="joinCart bnts" @tap="openAlone" v-if="storeInfo.product_is_show">{{ $t(`单独购买`) }}</view>
          <view class="buy bnts bg-color-hui" :class="!storeInfo.product_is_show ? 'long-btn' : ''">{{ $t(`已售罄`) }}
          </view>
        </view>
        <view class="bnt acea-row" v-if="!dataShow && status == 1">
          <view class="joinCart bnts" @tap="openAlone" v-if="storeInfo.product_is_show">{{ $t(`单独购买`) }}</view>
          <view class="buy bnts bg-color-hui" :class="!storeInfo.product_is_show ? 'long-btn' : ''">{{ $t(`立即购买`) }}
          </view>
        </view>
        <view class="bnt acea-row" v-if="status == 2">
          <view class="joinCart bnts" @tap="openAlone" v-if="storeInfo.product_is_show">{{ $t(`单独购买`) }}</view>
          <view class="buy bnts bg-color-hui" :class="!storeInfo.product_is_show ? 'long-btn' : ''">{{ $t(`未开始`) }}
          </view>
        </view>
        <view class="bnt acea-row" v-if="status == 0">
          <view class="joinCart bnts" @tap="openAlone" v-if="storeInfo.product_is_show">{{ $t(`单独购买`) }}</view>
          <view class="buy bnts bg-color-hui" :class="!storeInfo.product_is_show ? 'long-btn' : ''">{{ $t(`已结束`) }}
          </view>
        </view>
      </view>
    </view>
