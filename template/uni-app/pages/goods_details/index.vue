        ...
        <scroll-view :scroll-top="scrollTop" scroll-y="true" scroll-with-animation="true"
          ...
          <view id="past0">
            ...
            <productConSwiper ref="proSwiper" :imgUrls="storeInfo.slider_image" :videoline="storeInfo.video_link"
              @videoPause="videoPause" @showSwiperImg="showSwiperImg"></productConSwiper>
            <view class="wrapper">
              <view class="share acea-row row-between row-bottom">
                <view class="money font-color">
                  ...
                  <text class="vip-money"
                    v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.is_vip == 1 && svip_price_open == 1">
              ...
              <view class="limit_good" v-if="storeInfo.limit_type > 0">
                {{ storeInfo.limit_type == 1 ? $t(`单次限购`) : $t(`永久限购`) }}{{ storeInfo.limit_num }}{{
              ...
              <view v-if="couponList.length" class="coupon acea-row row-between-wrapper" @click="couponTap"
                ...
                <view class="hide line1 acea-row">
                  ...
                  <template v-for="(item, index) in couponList">
                    <view v-if="index < 2" class="activity" :key="index">{{ $t(`满`) }} {{ item.use_min_price }} {{
    $t(`减`) }} {{ item.coupon_price }}</view>
                  </template>
                </view>
                    ...
                    <view class="item" v-for="(val, indexn) in item.list" :key="indexn" @click="goDetail(val)">
                      <view class="pictrue">
                        ...
                        <span class="pictrue_log pictrue_log_class"
                          v-if="val.activity && val.activity.type === '1' && $permission('seckill')">{{ $t(`秒杀`)
                          }}</span>
                        <span class="pictrue_log pictrue_log_class"
                          v-if="val.activity && val.activity.type === '2' && $permission('bargain')">{{ $t(`砍价`)
                          }}</span>
                        <span class="pictrue_log pictrue_log_class"
                          v-if="val.activity && val.activity.type === '3' && $permission('combination')">{{ $t(`拼团`)
                          }}</span>
                      </view>
        ...
        <view v-else>
          ...
          <view class="presale" v-else>
            <view class="acea-row" v-if="presale_pay_status === 1 || presale_pay_status === 3">
              <form class="bnts bg-color-hui">
                <button class="bnts bg-color-hui" form-type="submit">{{ presale_pay_status === 1 ? $t(`未开始`) : $t(`已结束`)
                  }}</button>
              </form>
            </view>
            <view class="acea-row"
