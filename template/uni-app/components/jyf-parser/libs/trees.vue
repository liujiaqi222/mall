    ...
    <block v-for="(n, index) in nodes" v-bind:key="index">
      ...
      <template v-if="n.name == 'img'">
        <image v-if="allowScan" :id="n.attrs.id" class="_img" :style="n.attrs.style" :src="n.attrs.src"
          :data-attrs="n.attrs" mode="widthFix" :show-menu-by-longpress="true" @tap="() => { }" />
      ...
      <view v-else-if="n.name == 'video'">
        <view v-if="(!loadVideo || n.lazyLoad) && !(controls[n.attrs.id] && controls[n.attrs.id].play)" :id="n.attrs.id"
          :class="'_video ' + (n.attrs.class || '')" :style="n.attrs.style" @tap="_loadVideo" />
        <video v-else :id="n.attrs.id" :class="n.attrs.class" :style="n.attrs.style"
          ...
          :src="n.attrs.source[(controls[n.attrs.id] && controls[n.attrs.id].index) || 0]" :unit-id="n.attrs['unit-id']"
          :data-id="n.attrs.id" data-from="video" data-source="source" @error="error" @play="play" />
      </view>
      <!--音频-->
      <audio v-else-if="n.name == 'audio'" :class="n.attrs.class" :style="n.attrs.style" :author="n.attrs.author"
        ...
        :poster="n.attrs.poster" :src="n.attrs.source[(controls[n.attrs.id] && controls[n.attrs.id].index) || 0]"
      ...
      <trees v-else :class="(n.attrs.id || '') + ' _' + n.name + ' ' + (n.attrs.class || '')" :style="n.attrs.style"
        :nodes="n.children" :lazyLoad="lazyLoad" :loadVideo="loadVideo" :allowScan="allowScan" />
      <!--#endif-->
    </block>
  </view>
</template>
